...
export const queryOverpass = async (query: string): Promise<any> => {
  ...
  try {
    ...
    if (error instanceof TypeError && error.message === 'Failed to fetch') {
      ...
      return queryOverpassFallback(query);
    }
...
// Fallback to a different Overpass instance if the main one fails
const queryOverpassFallback = async (query: string): Promise<any> => {
